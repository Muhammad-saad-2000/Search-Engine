...
public class basicTextExtractor {
    ...
    public void run() {
        ...
        //Anything that is not a letter is a separator, a sequence of separators is one big separator
        String Separators = "[^\\w]+"  ;
        ...
        for(String word : rawWords){
            if( Filters[0].apply(word)
             && Filters[1].apply(word)
             && Filters[2].apply(word)) {
                        stemmedWords.add(
                                     stemmer.stem(word).toString().toLowerCase(Locale.ROOT)
                        ) ;
            }
        }
        ...
        for(String stemWord : stemmedWords){
            if(stemWordsFrequency.containsKey(stemWord)){
                stemWordsFrequency.put(stemWord,
                                       stemWordsFrequency.get(stemWord) + 1);
    ...
    private void persistIntoDB(String URL, String Title, String rawContent, Map<String,Integer> wordFrequencyTable){
        try{
            Connection conn = DriverManager.getConnection("jdbc:mysql://127.0.0.1:3306/index?allowMultiQueries=true","root","pcQ8cMA3d)DIO)Wn");
